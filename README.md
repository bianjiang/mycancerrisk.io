...
```
1. Git clone + HTTPS
   Copy the whole repository on your local machine
2. pip install -r requirements.txt
   Install python flask, pymongo and Flask-OAuthlib
3. cd static
   Go into 'static' folder
4. npm install -g bower
5. bower install
   Install all nodejs packges
```
...
#### MongoDB
   Install on Linux:
    ```
        ...
        4. $ sudo apt-get ins# CRC Risk Assessment Web Application
...
4. test on server_domain_or_IPtall -y mongodb-org
    ```
    Run MongoDB:
        1)  Start MongoDB
        * $ sudo service mongod start
...
#### Create a Self-Signed SSL Certificate for Nginx
    Step 1:Create the SSL Certificate
       ...
       * $ sudo openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048
       Step 2: Configure Nginx to Use SSL
           1) Create a Configuration Snippet Pointing to the SSL Key and Certificate
              a.First, let's create a new Nginx configuration snippet in the /etc/nginx/snippets directory.
              * $ sudo nano /etc/nginx/snippets/self-signed.conf
              b.Within this file, we just need to set the ssl_certificate directive to our certificate file and the ssl_certificate_key to the associated key. In our case, this will look like this:
              ```
              ssl_certificate /etc/ssl/certs/nginx-selfsigned.crt;
              ssl_certificate_key /etc/ssl/private/nginx-selfsigned.key;
              ```
           2) Create a Configuration Snippet with Strong Encryption Settings
              a. Create another snippet that will define some SSL settings
              * $ sudo nano /etc/nginx/snippets/ssl-params.conf
              ...
              ```
           3) Adjust the Nginx Configuration to Use SSL
              a.Back up our current server block file
              * $ sudo cp /etc/nginx/sites-available/default /etc/nginx/sites-available/default.bak
              b.Open the server block file to make adjustments
              * $ sudo nano /etc/nginx/sites-available/default
              c.Inside, modify server block
              ```
                #
                # You should look at the following URL's in order to grasp a solid understanding
                # of Nginx configuration files in order to fully unleash the power of Nginx.
